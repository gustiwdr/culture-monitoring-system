...
<div class="container mx-auto p-6 pl-64">
    ...
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-semibold text-gray-800">Aktivitas yang Menunggu Persetujuan Admin HC</h1>
    </div>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2 border">
                                @if ($activity->approval_status == 'approved_by_head')
                                    <form action="{{ route('admin.activities.approve-admin', $activity->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit" class="bg-green-500 text-white py-2 px-4 rounded hover:bg-green-400">Setujui</button>
                                    </form>

                                    <form action="{{ route('admin.activities.reject-admin', $activity->id) }}" method="POST" class="inline ml-4">
                                        @csrf
                                        @method('PUT')
